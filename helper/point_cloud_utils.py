...
def process_point_cloud_match_sdf(filename, param, normalization_params:dict):
    """
    ...
    if param.voxel_size is None:
        ...
        pcd = pcd_tmp.voxel_down_sample(
            voxel_size=param.voxel_size)  # downsample
        
    V = np.asarray(pcd.points)
    V_norm = (V - normalization_params["min_coords"]) / normalization_params["max_coord"]
    pcd.points = o3d.utility.Vector3dVector(V_norm)
...
def compute_tangent_space(neighbor_coords):
    ...
    tangent_vector1 = np.array(
        [-coefficients[1], coefficients[0], 0])  # u-axis
    if (
...
def project_points2tangent_space(
    ...
    projected_agent_positon = (
        agent_coords - np.dot(agent_coords - plane_point,
...
def get_gradient(
    ...
    grad_uv = get_gradient_3rd_degree_polynomial(
        uv_coords, coeffs, return_neighbors)
